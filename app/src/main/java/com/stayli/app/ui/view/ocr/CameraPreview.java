package com.stayli.app.ui.view.ocr;
...
import com.stayli.app.ui.face.FaceOnImageAvailableListener;
...
public class CameraPreview extends TextureView {
        ...
        private void process(CaptureResult result) throws CameraAccessException {
            switch (mState) {
                ...
                case STATE_WAITING_LOCK: {
                    ...
                    } else if (mIsLensFacing) { // 使用前置 需要检查是否支持自动对焦
                        CameraManager manager = (CameraManager) getContext().getSystemService(Context.CAMERA_SERVICE);
                        CameraCharacteristics characteristics = manager.getCameraCharacteristics(mCameraId);
                        int[] afAvailableModes = characteristics.get(CameraCharacteristics.CONTROL_AF_AVAILABLE_MODES);
                        for (int afAvailableMode : afAvailableModes) { // 不支持自动对焦 只会返回 0
                            Log.e(TAG, "process: afAvailableModes " + afAvailableMode);
                        }
                        // 不支持自动对焦
                        if (afAvailableModes.length == 1 && afAvailableModes[0] == CameraMetadata.CONTROL_AF_STATE_INACTIVE) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (mOnImageAvailableListener != null) {
                    mOnImageAvailableListener.onSaveHandler(mBackgroundHandler);
     ...
     */
    private int getOrientation(int rotation) throws CameraAccessException {
        Log.e(TAG, "getOrientation: " + rotation);
//        int i = ORIENTATIONS.get(rotation);
//        Log.e(TAG, "getOrientation1: " + (i - 90));
        CameraManager manager = (CameraManager) getContext().getSystemService(Context.CAMERA_SERVICE);
        CameraCharacteristics characteristics = manager.getCameraCharacteristics(mCameraId);
        int jpegOrientation = getJpegOrientation(characteristics, rotation);
        Log.e(TAG, "getOrientation1: jpegOrientation " + jpegOrientation);
    ...
    private int getJpegOrientation(CameraCharacteristics c, int deviceOrientation) {
        ...
        if (facingFront) {
            Log.e(TAG, "getJpegOrientation:deviceOrientation  " + deviceOrientation);
            deviceOrientation = -deviceOrientation;
        }
        Log.e(TAG, "getJpegOrientation:sensorOrientation  " + sensorOrientation);
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback captureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               ...
                                               @NonNull TotalCaptureResult result) {
                    if (mOnImageAvailableListener != null) {
                        mOnImageAvailableListener.onCaptureCompleted();
                    }
                    unlockFocus();
                }
            };
    ...
    // 是否使用前置
    public void setLensFacing(boolean isLensFacing) {
        mIsLensFacing = isLensFacing;
    }
     ...
     * ImageReader的回调对象
     */
    private FaceOnImageAvailableListener mOnImageAvailableListener;

    public void setOnImageAvailableListener(FaceOnImageAvailableListener listener) {
